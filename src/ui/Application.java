    ...
    public Application() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                map().setToolTipText(CreateTooltip(pos, p));
            }
        });
    }

    private String CreateTooltip(ICoordinate pos, Point p){
        List<MapMarker> list = getMarkersCovering(pos, pixelWidth(p));
        for (MapMarker G: list){ if (G instanceof MapMarkerMaker){ MapMarkerMaker item = (MapMarkerMaker)G;
            return String.format("<html><body><img src='%s'>"
                        + item.getStatus().getText() + "</body></html>", item.getUser().getBiggerProfileImageURL());
            }
        }
       return "Tweet N/A";
    }
